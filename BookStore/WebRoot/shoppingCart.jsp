...
<html>
  <head>
	...
	<style type="text/css">
		.jiesuan{
			background-image: url("image/jiesuan.jpg");
			width: 117px;
			height: 33px;
					...
					<s:iterator value="#session.shoppingBook" status="st">
						<ul class="shoppingBookUl">
							...
							<li class="bookAmount">
								...
								<input type="button" value="修改" onclick="updateBookAmount('<s:property value="bookId"/>')" class="button">
							</li>
							<li class="delete">
								<input type="button" value="删除" onclick="deleteBook('<s:property value="bookId"/>')" class="button">
							</li>
						</ul>
					</s:iterator>
					<ul class="shoppingBookUl">
						<li class="shoppingBookHead" style="background-color: #FFFFCC;color:#FF6600;">
							<span style="margin-left:440px;">
							商品总价：<s:property value="#session.totalMoney"/> 元
							<s:if test="%{#session.shoppingBook.size() > 0}">
							<input type="button" value="" onclick="addOrders()" class="jiesuan">
							</span>
						</s:if>
						</li>
					</ul>
